...
for(j in 1:1000){
  ...
  mat.str <- brd.mat.fitness(mat = mat.str,min.fert = min.fert,max.fert = max.fert,type = "uniform") # decline
  head(mat.str)
...
write.table(x = sim.info,file = "Output/simulation.25_500off.25_500_npar.sr1.ratio.combined.information.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
write.table(x = pool,file = "Output/estimation.25_500off.25_500_npar.sr1.ratio.combined.information.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
