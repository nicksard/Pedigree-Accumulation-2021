...
n_my.off <- c(25,50,100,250,500)
n_my.par <- c(100)
n_my.mates <- c(4)
n_my.sex.ratio <- c(1,2,4)
...
for(i in 1:nrow(my.params)){
  ...
  for(j in 1:100){
    ...
    mom.pool$type <- "mom"
    mom.pool$true_n <- ms1$n.mom
    mom.pool$true_off <- ms1$noff
    head(mom.pool)
    ...
    dad.pool$type <- "dad"
    dad.pool$true_n <- ms1$n.dad
    dad.pool$true_off <- ms1$noff
    head(dad.pool)
...
write.table(x = sim.info_all,file = "Output/simulation.npar100_100.sex.ratio.parents.separated.information.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
write.table(x = pool_all,file = "Output/estimation.npar100_100.sex.ratio.separated.information.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
