...
n_my.mates <- c(4)
n_my.type <- c("uniform","decline")

#making the df to for loop through
my.params <- expand.grid(n_my.mates,n_my.par,n_my.off,n_my.type)
colnames(my.params) <- c("all_mates","all_par","all_offs","all_types")
head(my.params)
...
for(i in 1:nrow(my.params)){
  ...
  for(j in 1:100){
    ...
    mat.str <- brd.mat.fitness(mat = mat.str,min.fert = min.fert,max.fert = max.fert,type = my.params$all_types[i])
    head(mat.str)
    ...
    all.pool$type <- my.params$all_types[i]
...
write.table(x = sim.info_all,file = "Output/simulation.npar25_500.noff25_500.parents.combined.UD.information.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
write.table(x = pool_all,file = "Output/estimation.npar25_500.noff25_500.parents.combined.UD.information.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
