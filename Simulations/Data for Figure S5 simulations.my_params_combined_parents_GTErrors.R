...
my.params <- expand.grid(n_my.mates,n_my.par,n_my.off,n_my.sex.ratio,n_a,n_b)
colnames(my.params) <- c("all_mates","all_par","all_offs","all_sex.ratio","a","b")
head(my.params)
...
for(i in 1:nrow(my.params)){
  ...
  for(j in 1:100){
    ...
    ped2$off <- as.character(ped2$off)
    ped2$mom <- as.character(ped2$mom)
    ped2$dad <- as.character(ped2$dad)
    head(ped2)
    ...
    #first determining the rate parents failed to assign when present and applying to both sexes
    a1 <- my.params$a[i]*(1-my.params$b[i])
    ped3$m1 <- sample(x = c(0,1),size = nrow(ped3),replace = T,prob = c((1-a1),a1))
    ped3$d1 <- sample(x = c(0,1),size = nrow(ped3),replace = T,prob = c((1-a1),a1))
    ped3
    
    #now determine the rete of false assignments (b) and applying that to both sexes
    b1 <- my.params$b[i]
    ped3$m2 <- sample(x = c(0,1),size = nrow(ped3),replace = T,prob = c((1-b1),b1))
    ped3$d2 <- sample(x = c(0,1),size = nrow(ped3),replace = T,prob = c((1-b1),b1))
    
    #now looking for A&B errors
    ped3$m12 <- ped3$m1+ped3$m2
    ped3$d12 <- ped3$d1+ped3$d2
    table(ped3$m12)
    table(ped3$d12)
    
    #now making alterations to the pedigree
    
    #identifying ids
    mom.ids <- ped3$mom
    dad.ids <- ped3$dad
    
    #false assignments to unassigned moms or dads
    mom.locs2 <- which(ped3$m2 == 1)
    if(length(mom.locs2)>0){ped3$mom[mom.locs2] <- sample(x = mom.ids,size = length(mom.locs2),replace = F)}
    dad.locs2 <- which(ped3$d2 == 1)
    if(length(dad.locs2)>0){ped3$dad[dad.locs2] <- sample(x = dad.ids,size = length(dad.locs2),replace = F)}
    
    #unassigned moms or dads
    mom.locs1 <- which(ped3$m1 == 1)
    if(length(mom.locs1)>0){ped3$mom[mom.locs1] <- "UK"}
    dad.locs1 <- which(ped3$d1 == 1)
    if(length(dad.locs1)>0){ped3$dad[dad.locs1] <- "UK"}

    #false assignments to unassigned moms or dads for parents where they were unassigned
    mom.locs3 <- which(ped3$m12 == 2)
    if(length(mom.locs3)>0){ped3$mom[mom.locs3] <- sample(x = mom.ids,size = length(mom.locs3),replace = F)}
    dad.locs3 <- which(ped3$d12 == 2)
    if(length(dad.locs3)>0){ped3$dad[dad.locs3] <- sample(x = dad.ids,size = length(dad.locs3),replace = F)}
    ped3[dad.locs3,]
    ...
    #getting estimates for data
    all.pool1 <- specpool(all.com)
    all.pool1$true_n <- ms1$n.par
    all.pool1$true_off <- ms1$noff
    all.pool1$errors <- "Yes"
    head(all.pool1)
    
    #combining species accumulation estimates for graphics and evaluation
    pool1 <-rbind(all.pool,all.pool1)
    pool1$type_a <- my.params$a[i]
    pool1$type_b <- my.params$b[i]
...
write.table(x = sim.info_all,file = "Output/simulation.npar100_50_500.sex.ratio.parents.combined.WithError.information.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
write.table(x = pool_all,file = "Output/estimation.npar100_50_500.sex.ratio.parents.combined.WithError.information.txt",append = F,quote = F,sep = "\t",row.names = F,col.names = T)
